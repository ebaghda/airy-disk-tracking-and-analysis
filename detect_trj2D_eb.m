%function for trajectory linking
function T_fr = detect_trj2D(c_array,dc,dfr,Nfr_min)
...
wb = waitbar(0, "Linking trajectories");
for i=2:numel(c_array) % frames
    waitbar(i/numel(c_array), wb, "Linking trajectories")
    for j=1:size(c_array{i},1) % particles
        for k=1:dfr
            ...
            [M,ind]=min(sqrt(sum( (c_array{i}(j,1:2)-c_array{i-k}(:,1:2) ).^2,2)));
...
try
    ...
    T_ext.xy=cat(1,T.xy{:});
    %warning("Error in linking trajectories")
    %disp(ME.message); disp(ME.stack.line);
end
